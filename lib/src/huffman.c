...
int block_count = 0;
unsigned long bit_buffer = 0UL;
int bit_count = 0;
char block_bin_code[65];
...
void huff_encode(char *in_file, char *out_file){
	...
	bit_buffer = bit_buffer << (64 - bit_count);
	unsigned char remainder = 64 - bit_count;
	if(remainder < 64){
		write(out_fd, &bit_buffer, sizeof(unsigned long));
		/*dec_to_bin(block_bin_code, bit_buffer, 64);
		printf("%s\n", block_bin_code);*/
		block_count++;	
	}
...
void huff_decode(char *in_file, char *out_file){
	...
	unsigned long *encoded_huff_tree = malloc(huff_tree_blocks * sizeof(unsigned long));
	unsigned long *tmp = encoded_huff_tree;
	if((rb = read(in_fd, encoded_huff_tree, huff_tree_blocks * sizeof(unsigned long))) < 0){
