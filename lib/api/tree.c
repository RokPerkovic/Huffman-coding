...
char block_bin_code1[65];
int max_len = 0;

//unsigned int tree_block;
unsigned long content_block;
...
huff_node *rebuild_huff_tree(unsigned long **encoded_huff_tree){
	//printf("call...\n");
	
	if(read_bit(*encoded_huff_tree, &mask) == 1){
		mask = mask >> 1UL; // place the mask bit on the first bit of the following character
		if(mask == 0){
		...
		int16_t chr;
		if((chr = make_char(encoded_huff_tree, &mask)) < 0){ //read next 16 bits and construct a character
			fprintf(stderr, "Error rebuilding huff_tree!\n");
			exit(1);
		} 
...
void encode_huff_chars(huff_node *root, unsigned int *h_code /*char *h_code*/, int parent){
	if(root->left){
		*h_code = *h_code << 1 | 0;
	...
	if(!(root->left) && !(root->right)){
		...
		root->c->h_code = *h_code;
		
		/*dec_to_bin(block_bin_code1, *h_code, 32);
		printf("%c...%s, len: %d\n",root->c->c, block_bin_code1, root->c->h_code_len);*/
		/*if(parent > max_len){
			max_len = parent;
			printf("max len: %d\n", max_len);
		}*/
...
void encode_huff_tree(huff_node *root, unsigned long *bit_buffer, int *bit_count, int *block_count, int out_fd){
	if(root == NULL){
		return;
	}
	...
	if(!(root->left) && !(root->right)){
		//leaf
		write_bits(1UL, 1, bit_buffer, bit_count, block_count, out_fd);
		write_bits(root->c->c, 16, bit_buffer, bit_count, block_count, out_fd);
	}
	else{
		//inner node
		write_bits(0UL, 1, bit_buffer, bit_count, block_count, out_fd);
	}
...
void encode_content(unsigned long *bit_buffer, int *bit_count, int *block_count, int in_fd, int out_fd){
	int n;
	while((n = read(in_fd, input_buff, BUFF_SIZE)) > 0){
		if(n < 0){
			perror("Error reading input file...");
	...
	int remainder = BIT_BUFFER_SIZE - *bit_count;
	
	if(remainder < BIT_BUFFER_SIZE){
		*bit_buffer = *bit_buffer << (BIT_BUFFER_SIZE - *bit_count);
		write(out_fd, bit_buffer, sizeof(unsigned long));
	
		/*dec_to_bin(block_bin_code1, *bit_buffer, BIT_BUFFER_SIZE);
		printf("%s\n", block_bin_code1);*/
	}
}
...
int16_t decode_char(huff_node *root, unsigned long *mask, int in_fd){
...
void decode_content(huff_node *root, unsigned long *mask, int in_fd, char *output_buff, int *buff_pos, int out_fd){
